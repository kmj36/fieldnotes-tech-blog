...
func (h *BaseHandler) respondProcessError(ctx *gin.Context, err error) {
	...
	case errors.Is(err, dto.CErrFileTooLarge):
		ctx.JSON(dto.ErrBadRequestRange.Status, dto.ResponseWrapper[any]{
			Status:    dto.ErrBadRequestRange.Status,
	...
	case errors.Is(err, dto.CErrForbidden):
		ctx.JSON(dto.ErrForbidden.Status, dto.ResponseWrapper[any]{
			Status:    dto.ErrForbidden.Status,
			Code:      dto.ErrForbidden.Code,
			Detail:    err.Error(),
			Message:   dto.ErrForbidden.Message,
			Timestamp: time.Now().UTC(),
			Path:      ctx.Request.URL.Path,
		})
	default:
